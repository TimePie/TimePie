...
- (void) presentModallyOverViewController: (UIViewController*) controller withItem:(TimingItem*) item
{
	...
	UIBarButtonItem *deleteItemButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(cancel:)];
    self.navigationItem.leftBarButtonItem = deleteItemButton;
    
	[controller presentViewController: nav animated: YES completion: nil];
    if(item){
        timingItem = item;
        [textName setText:[timingItem itemName]];
        [self setSourceColor:[timingItem color]];
...
//	[self.delegate colorPickerControllerDidFinish: self];
    [self.delegate colorPickerControllerDidFinish: self nameText: [textName text]];
}


- (IBAction) editDone: (id) sender
{
    [timingItem setItemName:[textName text]];
    [timingItem setColor:[self resultColor]];
    //	[self.delegate colorPickerControllerDidFinish: self];
    [self.delegate colorPickerControllerDidFinish: self nameText: [textName text]];
}
